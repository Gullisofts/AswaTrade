    ...
    <div class="section_filter">
      <div class="container-fluid">
        <button class="btn btn-primary resfilter" id="resfilter">
          <h6 class="text-center m-0">فلترة المنتجات</h6>
          &nbsp;
...
export default {
    ...
    created:function()
    {
        ...
        $("#normalviewsection").fadeIn(0);

        ////////////////// Normal Show ////////////////////
        /////////////////////////////////////////////////////////
        axios.get("/getsectionproducts/"+self.sectionid+"__"+self.lastprodid)
        .then(function(response)
        {
          ...
          $("#normalviewsection").attr("disabled", false);
      ...
      { 
          ...
          else
          {
              ...
              self.allshown = "false";
              let psectionid = self.sectionid;
              axios.post("/filtersectiondata", {models_arr, price_arr, rating_arr, psectionid})
              .then(function(response)
              {
                if(response.data == "")
                {
      ...
      $(document).on("click", "#filterviewsection", function()
      {
        ...
        //check if All Elements Has been Got
            ...
            axios.post("/filtersectiondata/", {psectionid, last_filter_ele, price_arr, models_arr, rating_arr})
            .then(function(response)
            {
                if(response.data.length == 2)
                {
      ...
      $(document).on("click", "#filter_abolish", function()
      {
        ...
        .then(function(response)
        {
          ...
          {
            $("#normalviewsection").html("أعرض المزيد ...");
            self.allshown = "false";
    ...
    mounted:function()
    {
      ...
      //Get Filter Data
      axios.get("/getmanus/" + self.sectionid)
      .then(function(response)
      { 
